...
% fit to Lieleg model;
zlg = input('Fit Lieleg model? ');

if zlg == 1;
%Wfrf_lg=[50 1e-12 1 1e-12 1e-12];%% weight for transfer function, phase  and power, Glieleg,G"lileg
Wfrf_lg=[20 1e-12 1 1e-12 1e-12];
options_lg = optimset('TolFun',1e-10,'TolX',1e-9,'Display','iter','MaxFunEvals',1e5,'MaxIter',2e3);%,'DiffMinChange',1e-1);
%% ulg = beta   alpha   k_trap  koff    f0      G0    eta     ck     dk]
%Norm=   [u2(1)  u2(3)   u2(4)   1      1     10     5      1  10]; %%K255E
%Norm = [u2(1)   u2(3)   u2(4)   5      1       10  5       1   10]; %WT
Norm = [u2(1)   u2(3)   u2(4)   .1      10       10  5       1   10]; %WT
ulg0 = ones(size(Norm));
%lb_lg = [0.9    0.9     0.9     1e-1    0.1   1e-2    0.1    1e-1   1e-1]; 
%ub_lg = [1.1    1.1     1.1     10      10     1e1     2    1e1   1e1];
lb_lg = [0.9 0.9 0.9 0.01 0.01   0 0.5   0 0];
ub_lg = [1.1 1.1 1.1 10 10 1e2 1e1 1e6 1e6];
...
figure(hfrf);
disp('Min. frequency to fit: ')
[flg_min,ylg_min]=ginput(1);
disp('Max. frequency to fit: ')
[flg_max,ylg_max]=ginput(1);

jlg = find(Fexcall>flg_min & Fexcall<flg_max);
Fexcall = Fexcall(jlg);
Hall = Hall(jlg);
Call = Call(jlg);

xdata_lg=[Fexcall,Fexcall,FP1_p',Fexcall,Fexcall];
ydata_lg=[zeros(size(Hall)),zeros(size(Hall)),zeros(size(FP1_p))',zeros(size(Fexcall)),zeros(size(Fexcall))];
fun1=@(u1,ydata1)theor_trap_frf_nove_rlieleg_fischer_iterative_AGH(u1,Fexcall,abs(Hall),angle(Hall),FP1_p,PYm1_p, Rbead,kT,Wfrf_lg,Norm);
[u2lg,resnorm1,residual1]=lsqcurvefit(fun1,ulg0,xdata_lg,ydata_lg,lb_lg,ub_lg,options_lg);


[Mfrf_lg0, PHfrf_lg0] = frf_lieleg_fischer_AGH(ulg0,Fexcall,Rbead,kT,Norm);
PS_lg0 = ps_lieleg_fischer_AGH(ulg0,ff,Rbead,kT,Norm);
[Mfrf_lg,PHfrf_lg]=frf_lieleg_fischer_AGH(u2lg,Fexcall,Rbead,kT,Norm);
PS_lg=ps_lieleg_fischer_AGH(u2lg,ff,Rbead,kT,Norm);

figure, hfrflg = gcf;
subplot(311), semilogx(Fexcall,20.*log10(abs(Hall)),'bo')
subplot(312), semilogx(Fexcall,(180/pi).*angle(Hall),'bo')
subplot(313), semilogx(Fexcall,Call,'bo')

figure, hpslg = gcf;
loglog(FP1_p,PYm1_p,'b')

figure(hfrflg), subplot(311), hold on,
set(gca,'fontsize',14,'fontweight','bold')
semilogx(Fexcall,20.*log10(Mfrf_lg0),'g')
semilogx(Fexcall,20.*log10(Mfrf_lg),'m','linewidth',2)
subplot(312), hold on,
semilogx(Fexcall,(180/pi).*PHfrf_lg0,'g')
semilogx(Fexcall,(180/pi).*PHfrf_lg,'m','linewidth',2)%+(180/pi).*unwrap(angle(exp(-j*1e-6.*Hw)))
set(gca,'fontsize',14,'fontweight','bold')

figure(hpslg), hold on
loglog(ff,PS_lg0,'g')
loglog(ff,PS_lg,'m','linewidth',2)

[h,Gp_final,Gpp_final]=Gplot_lieleg_fischer_AGH(u2lg,ff,Rbead,kT,Norm);

figure, 
loglog(ff,Gp_final,'b') %%%????
hold on;
loglog(ff,Gpp_final,'r') %%%???
xlabel freq;
ylabel [Gp,Gpp]

ulg=u2lg.*Norm;

disp(['FINAL FIT: Lieleg -------------------------------'])
%disp(['Dfit = ', num2str(Dfit), ' V'])
%disp(['f_c,zero = ', num2str(fc_zero), ' Hz'])
%disp(['f_c,FRF = ', num2str(fc_frf), ' Hz'])
disp(['Beta = ', num2str(ulg(1)), ' nm/V'])
%disp(['Gamma = ',num2str(gammar),' *gamma0'])
disp(['Alpha = ',num2str(ulg(2))])
disp(['k_trap = ',num2str(ulg(3)),' pN/nm'])
disp(['k_off = ', num2str(ulg(4)), ' 1/s'])
disp(['f_0 = ',num2str(ulg(5)),' Hz'])
disp(['G_0 = ',num2str(ulg(6)),' Pa'])
disp(['eta = ',num2str(ulg(7))]);
disp(['ck = ',num2str(ulg(8))]);
disp(['dk = ',num2str(ulg(9))]);
